...
@app.route('/<post_title>/update',methods=['GET','POST'])
@login_required
def update_post(post_title):
    post_to_update = post.query.filter_by(Title=post_title).first()
    form = new_post()
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        return redirect(url_for('read_post',post_title=post_to_update.Title))
